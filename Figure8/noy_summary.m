...
for t = 1:numel(treatments)
    for p = 1:numel(phases)
        ...
        for f = 1:numel(cdc14_files)
            ...
            nuc_filename = strrep(rdna_filename, 'Cdc14_GFP.tif', 'Cbf5_RFP.tif');
            [ ...
...
for m = 1:numel(metrics)
    ...
    if contains(metrics{m}, 'nuc')
        ...
        b.FaceColor = 'flat';
        b.CData = [repmat([178/255, 34/255, 34/255], [3,1]); repmat([128/255, 0, 0], [3,1])];
        b.CData(4:end, :) = repmat([128/255, 0, 0], [3,1]);
    elseif contains(metrics{m}, 'rdna')
        b = bar([m_g1(:,1); m_m(:,1)]);
        b.FaceColor = 'flat';
        b.CData = [repmat([154/255, 205/255, 50/255], [3, 1]); repmat([0/255, 128/255, 0/255], [3, 1])];
    else
        b = bar([m_g1(:,1); m_m(:,1)]);
        b.FaceColor = 'flat';
        b.CData = [repmat([0/255, 178/255, 178/255], [3,1]); repmat([0/255, 128/255, 128/255], [3, 1])];
    end
    hold on;
    ...
    xticklabels(...
        {'G1, WT', 'G1, pNOY TX On', 'G1, pNOY TX Off', ...
        'G2/M, WT', 'G2/M, pNOY TX On', 'G2/M, pNOY TX Off'});
